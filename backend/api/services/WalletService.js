...
var Atomic = require('../executors/Atomic');
...
module.exports = {
  ...
  transfer: async function (senderCustomerId, receiverPhone, amount) {
    ...
    try {
      ...
      var transferResult = await Atomic.TransferTransaction(sender.id, receiver.id, amount);
      return {
        transaction: {
          ...
          status: transferResult.transaction.status
        },
        senderPocket: {
          id: transferResult.senderPocket.id,
          balance: transferResult.senderPocket.balance
        }
      };
    } catch (error) {
      if (error instanceof AppError) {
